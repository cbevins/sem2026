/**
 * The FuelModelProcessor accepts a fuel model key and returns a copy of the associated
...
export class FuelModelProcessor {
    static Inputs = [
        {key: 'fuelKey', desc: 'FuelModel number or alpha key',
     ...
     */
    get(inputs={}) {
        if (Object.hasOwn(inputs, 'fuelKey')) {
            this.fuelKey = inputs.fuelKey
            const fuelModel = this.catalog.get(this.#toKey(this.fuelKey)) 
            return (fuelModel === undefined) ? false : this.#clone(fuelModel)
        }
        return false
    }
     ...
     */
    has(inputs={}) {
        return (Object.hasOwn(inputs, 'fuelKey'))
            ? this.catalog.has(this.#toKey(inputs.fuelKey)) : false
    }
