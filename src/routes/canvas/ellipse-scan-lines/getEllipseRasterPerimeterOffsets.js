 ...
 * @returns An array of fire ellipse perimeter raster coordinates expressed as integer offsets
 * from the ignition point (origin).
...
export function getEllipseRasterPerimeterOffsets(cx, cy, rx, ry, rotationDegrees, spacing, limit=10000) {
    ...
    let px = cx // + ry
    let py = cy // + ry
    while (insideEllipse(px, py+s, cx, cy, rx, ry, rotRad)) {
        py += spacing
    }
    let point = [px, py]
    store(point)
    ...
    while(! done) {
        ...
        if (travel === 'north') {   // test sequence is north -> northeast -> east
            if (insideEllipse(px, py+s, cx, cy, rx, ry, rotRad)) {  // check to the north
                py = py + s
                point = [px, py]
            } else if (insideEllipse(px+s, py+s, cx, cy, rx, ry, rotRad)) { // check to the north-east
                px = px + s
                py = py + s
                point = [px, py]
            } else if (insideEllipse(px+s, py, cx, cy, rx, ry, rotRad)) { // check to the east
                px = px + s
                point = [px, py]
            } else {
                travel = 'east'
            ...
            if (insideEllipse(px+s, py, cx, cy, rx, ry, rotRad)) { // check to the east (redundant of coming from north)
                px = px + s
                point = [px, py]
            } else if (insideEllipse(px+s, py-s, cx, cy, rx, ry, rotRad)) { // check to the south-east
                px = px + s
                py = py - s
                point = [px, py]
            } else if (insideEllipse(px, py-s, cx, cy, rx, ry, rotRad)) { // check to the south
                py = py - s
                point = [px, py]
            } else {
                travel = 'south'
            ...
            if (insideEllipse(px, py-s, cx, cy, rx, ry, rotRad)) { // check to the south (may be redundant)
                py = py - s
                point = [px, py]
            } else if (insideEllipse(px-s, py-s, cx, cy, rx, ry, rotRad)) { // check to the south-west
                px = px - s
                py = py - s
                point = [px, py]
            } else if (insideEllipse(px-s, py, cx, cy, rx, ry, rotRad)) { // check to the west
                px = px - s
                point = [px, py]
            } else {
                travel = 'west'
            ...
            if (insideEllipse(px-s, py, cx, cy, rx, ry, rotRad)) { // check to the west (redundant when coming from the south)
                px = px - s
                point = [px, py]
            } else if (insideEllipse(px-s, py+s, cx, cy, rx, ry, rotRad)) { // check to the north-west
                px = px - s
                py = py + s
                point = [px, py]
            } else if (insideEllipse(px, py+s, cx, cy, rx, ry, rotRad)) { // check to the north
                py = py + s
                point = [px, py]
            } else {
                travel = 'north'
