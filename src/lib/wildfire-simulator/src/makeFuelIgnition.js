...
export function makeFuelIgnition(inputs={}) {
    ...
    for(let life of ["dead", "live"]) {
        ...
        for(let particle of fuelBed[life].particles) {
            if(! Object.hasOwn(fuelMoisture, particle.moistureClass)) {
                throw new Error(`makeFuelIgnition(): a fuel particle has a moisture class '${particle.moistureClass}' that is not in the fuelMoisture object.`)
            }
            const moistureContent = fuelMoisture[particle.moistureClass]
