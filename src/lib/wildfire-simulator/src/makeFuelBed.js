 ...
 */
export function makeFuelBed(inputs={}) {
    const {fuelModel} = inputs
    const dead = makeFuelBedLife('dead', inputs)
    dead.mext = fuelModel.deadMext
    const live = makeFuelBedLife('live', inputs)
 ...
 */
function makeFuelBedLife(category, inputs) {
        ...
        if (particle.life === category || particle.life === "curable") {
            ...
            if (particle.life === "curable") {
                ...
                if (!Object.hasOwn(fuelCuring, particle.curingClass))
                    throw  new Error(`makeFuelBed() fuel model ${fuelModel.number} has a curingClass ${particle.curingClass} that was not provided as input.`)
                let curedFraction = fuelCuring[particle.curingClass]
