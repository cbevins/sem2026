 ...
 * ratio and (2) head fire spread ratthis.  These two parameters distinguish one FireEllipse
 * instance from another, and determine properties such as eccentricity, backing spread rate,
 * length and width expansion ratthis.
 * 
 * FireEllipse dimensions of interest (length, width, major and minor radii, perimeter, area)
 * at any time are updatable by calling the setDuration(duration) method.
 * 
 * The FireEllipse geographic locations of interest (ignition, center, head, back, flank, perimeter)
...
export class FireEllipse {
    ...
    setDuration(duration) {
        ...
        this.fDist = this.fRos * this.duration
        this.gDist = this.gRos * this.duration
        this.hDist = this.hRos * this.duration
