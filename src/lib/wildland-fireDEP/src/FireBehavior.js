...
import { WindSlopeConditionsTemplate } from './WindSlopeConditionsTemplate.js'
import { FireBehaviorConfigDefault } from './FireBehaviorConfigDefault.js'

export class FireBehavior {
    constructor(fuelIgnition, windSlopeConditions={}, behaviorConfig={}, config={}) {
        this.config = {...ConfigTemplate, ...config}
        this.behaviorConfig = {...FireBehaviorConfigDefault, ...behaviorConfig} 
        this.windSlopeConditions = {...WindSlopeConditionsTemplate, ...windSlopeConditions}
        this.setWindSlopeConditions(fuelIgnition, this.windSlopeConditions,
            this.behaviorConfig)
    }

    setWindSlopeConditions(fuelIgnition, windSlopeConditions={}, behaviorConfig={}) {
        this.behaviorConfig = {...FireBehaviorConfigDefault, ...behaviorConfig} 
        ...
        const {midflameWindSpeed, windBearing, aspect, slopeRatio} = windSlopeConditions
        this.midflameWindSpeed = midflameWindSpeed  // need this for getScorchHeight()
        ...
        const windFactor = (midflameWindSpeed > 0) ? windK * Math.pow(midflameWindSpeed, windB) : 0
        ...
        //----------------------------------------------------------------------
        let p
        if (this.behaviorConfig.limitSpreadRateByReactionIntensity) 
            p = this.behaviorConfig.limitSpreadRateByEffWindSpeed ? p7 : p5
        else
            p = this.behaviorConfig.limitSpreadRateByEffWindSpeed ? p6 : p3
    ...
    getScorchHeight(airTemp) {
        ...
        return (this.firelineIntensity > 0) ?
            ((63 / (140 - airTemp)) * Math.pow(this.firelineIntensity, 1.166667)) /
