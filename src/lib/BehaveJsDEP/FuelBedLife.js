export class FuelBedLife {
    constructor(category, fuelModel, curingConditions={}) {
    ...
    setFuelConditions(fuelModel, curingConditions={}) {
        ...
        for(let particle of fuelModel.particles) {
            ...
            if (particle.life === this.category || particle.life === "curable") {
                ...
                const surfaceArea = (particle.density > 0)
                    ? (particle.ovendryLoad * particle.savr) / particle.density : 0
                lifeSurfaceArea += surfaceArea

                // Determine particle's size class [0-5]
                const sizeClass = getFuelParticleSizeClass(particle.savr)
        ...
        // Store only those properties needed down stream
...
export class DeadFuelBed extends FuelBedLife{
    constructor(fuelModel, curingConditions={}) {
        super("dead", fuelModel, curingConditions)
...
export class LiveFuelBed extends FuelBedLife {
    constructor(fuelModel, curingConditions={}) {
        super("live", fuelModel, curingConditions)
        this.mext = 5       // rederived by parent FuelBed
    }
}

// Returns a size class index [0-5]
export function getFuelParticleSizeClass(savr) {
