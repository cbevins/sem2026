...
const config = {saveInfoProps: false, saveTestProps: false}
const catalogConfig = { FBFM13: true, FBFM40: true, LANDFIRE: false, CUSTOM: false, COWN: false}
const catalog = new StandardFuelModelCatalog(catalogConfig)
...
function runThruFuelIgnition(reps=10000) {
    ...
    for(let rep=0; rep<reps; rep++) {
        for(let modelKey of modelKeys) {
            const fuelModel = catalog.get(modelKey)
            const fuelBed = new FuelBed(fuelModel, curingConditions)
            const fuelIgnition = new FuelIgnition(fuelBed, moistureConditions)
