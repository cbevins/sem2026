<script>
    ...
    let {chart, yvar='flame'} = $props()
    let data = $derived(buildData(chart))
    ...
    function buildData(chart) {
        ...
        for(let fuelKey of chart.fuelKeys) {
            if (chart.isActive(fuelKey)) {
                d.push({
                    fuelKey,
                    groupKey: chart.groupKey(fuelKey),
                    spriteFill: chart.groupSpriteFill(fuelKey),
                    label: chart.label(fuelKey),
                    ros: chart.ros(fuelKey),
                    fli: chart.fli(fuelKey),
                    flame: chart.flame(fuelKey),
                })
            }
        }
        return d
    ...
    function updateSprites(data) {
        ...
        for(let item of data) {
            ...
            d.push({label: item.label, x, y, fill: item.spriteFill, group: item.groupKey})
        }
        // console.log(d)
        return d
    }
